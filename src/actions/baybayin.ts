const baybay_consonants = [
  5898, 5891, 5895, 5892, 5905, 5902, 5899, 5896, 5897, 5904, 5894, 5903, 5900,
];

const baybayin_ng = 5893;

const baybay_vowels = [5906, 5907, 5908];

const baybayin_i_vowels = [5888, 5889, 5890];
...
export default async function baybayin(data: string) {
  ...
  data = data
    .toLowerCase()
    .replace(/juan/gi, "huwan")
    .replace(/jesus/gi, "hesus")
    .replace(/josue/gi, "hoswe")
    .replace(/mateo/gi, "mateyo")
    .replace(/timoteo/gi, "timoteyo")
    .replace(/ezekiel/gi, "ezekiyel");
  data = data
    .replace(/i/gi, "e")
    .replace(/u/gi, "o")
    .replace(/r/gi, "d")
    ...
    .replace(/x|z/gi, "s")
    .replace(/j/gi, "dy")
    .replace(/\sng\s/gi, " nang ")
    .replace(/\.|\?|!/gi, String.fromCharCode(5942))
